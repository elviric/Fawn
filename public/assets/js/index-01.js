...
function initContract(cAdd,cABI){
  ...
  document.getElementById("apply").addEventListener("click", function(){
      var numTKN = $('input[name=numOfToken]').val();
      var t_value = $('input[name=tokenValue]').val();
      console.log(numTKN+ ' '+ t_value);

      if(numTKN > 0 ){
        var fee_value = web3.toWei(0.01, 'ether');
        contractInstance.createProduct(numTKN,t_value,{from: web3.eth.accounts[0], gas: 2000000, value: fee_value}, function(error,result){
          if(!error){
            console.log(result);
            txnHash = result;
  ...
  document.getElementById("Buy").addEventListener("click", function(){
    ...
    //  var t_value = $('input[name=tokenValue]').val();
      ...
      if(numTKN > 0 ){
        var fee_value = web3.toWei(0.01, 'ether');
        contractInstance.createProduct(TKNnum,{from: web3.eth.accounts[0], gas: 2000000, value: fee_value}, function(error,result){
          if(!error){
            console.log(result);
            txnHash = result;
